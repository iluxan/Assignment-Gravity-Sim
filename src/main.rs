...
async fn main() {  // This is the function that is normally set to immediately execute on starting the program. 
	...
	planetary_bodies.push(PlanetaryBody {mass: 2.0, radius: (macroquad::prelude::screen_height() as f64) / 80.0, velocity: [30.0, -100.0], location: [{1.0-{macroquad::prelude::screen_width() * 0.0625}} as f64, -10.0], colour: macroquad::prelude::PURPlE});
	//temporal_random_f32();
	loop {
